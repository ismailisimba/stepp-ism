  ...
  <v-container fluid>
    ...
    <v-container fluid v-else>
      <v-stepper v-model="step" mobile-breakpoint="sm" :items="steps">
        ...
        <template v-slot:item.2 v-for="section in sections">
          <v-container fluid>
            <QuestionInput
...
import QuestionInput from "../solicitation/components/QuestionInput.vue";
import {
  getReport,
  getReportQuestions,
  updateReport,
...
const steps = computed(() => {
  ...
  if (sections.value.length > 0)
    for (let index = 0; index < sections.value.length; index++) {
      if (sections.value[index].title)
        s.unshift(sections.value[index].title as string);
      else s.unshift(`${t("section")} ${index + 1}`);
    }
  s.unshift(t("reportInfo"));
  return s;
});
