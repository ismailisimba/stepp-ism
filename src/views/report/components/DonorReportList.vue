  ...
  >
    ...
    <template v-slot:item.actions="{ item }">
      <v-icon
        ...
        @click="
          router.push({ name: 'reports.edit', params: { reportId: item.id } })
        "
      >
      </v-icon>
      <v-icon
        ...
        @click="
          router.push({ name: 'reports.show', params: { reportId: item.id } })
        "
      >
      </v-icon>
      <!-- <v-icon
...
const headers = [
  { title: t("name"), key: "name", sortable: false, width: "50%" },
...
const deleteItem = (itemId: string, itemName: string) => {
  deleteUrl.value = apiEndpoints.reportDelete;
